...
const Login = ({setUserNameLogin,logout}) => {
  ...
  const login = async () => {
    try {
      ...
      userCredential.user.displayName && setUserNameLogin(userCredential.user.displayName)
      navigate('/')
    } catch (error) {
  ...
  return (
    ...
    <div
     ...
     Log In
        ...
        <div>
          <input name="floating_email" type="email" onChange={(e)=>setLoginEmail(e.target.value)} />
          <label htmlFor="floating_email">Email</label>
        </div>
        <div>
          <input name="floating_password" type="password" onChange={(e)=>setLoginPassword(e.target.value)} />
          <label htmlFor="floating_password">Password</label>
          <div className="flex justify-between links-a font-[0.75em] cursor-pointer decoration-none text-[#8f8f8f] mt-3">
            <span role="button" className="hover:text-red-main">Forgot Password</span>
            <Link to={"/register"} className="text-red-main">Sing Up</Link>
          </div>
        </div>
